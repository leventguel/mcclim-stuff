...
(defvar *maxima-userdir* "/home/wbooze/.maxima/")
(defvar *maxima-tempdir* "/home/wbooze/temp/")


(defparameter *x-width* nil)
(defparameter *y-width* nil)
...
(define-application-frame maxima-repl (standard-application-frame)
  ...
  (:panes
   (interactor :interactor
     ...
     :background +gray10+ :foreground +goldenrod+)
...
(defun maxima-prompt (stream frame)
  ...
  (with-drawing-options (stream :text-size :huge :ink +brown+)
...
#+nil
(eval-when (:compile-toplevel :load-toplevel :execute)
  (define-presentation-type lisp-expression () :inherit-from t))

(define-presentation-method present (object (type maxima-expression)
					    stream (view textual-view)
					    &key &allow-other-keys)
  (with-drawing-options (stream :text-size :huge :text-face :bold :ink +olivedrab+)
  (cond
    (maxima::*alt-display2d*
     ...
     (let ((*standard-output* stream)
	   (maxima::$display2d t)
	   (maxima::*y-width* (* 2 (text-style-height (medium-text-style stream)
	   					 stream)))
	   (maxima::*x-width* (* 1/4 (text-style-width (medium-text-style stream)
	   					stream))))
...
#+nil
(define-presentation-method present (object (type lisp-expression)
					    stream (view textual-view)
					    &key &allow-other-keys)
  (with-drawing-options (stream :text-size :huge :text-face :bold :ink +olivedrab+)
  (cond
    (maxima::*alt-display2d*
     ...
     (let ((*standard-output* stream)
       ...
       ;;       maxima::*x-width* maxima::*y-width*)
       (let ((r (with-new-output-record (t)
		  (cl-user::display-formula stream object))))
	 r)))
    (t
      ;; This method works and scrolling is good.
      (princ (untabify (with-output-to-string (s)
...
#+nil
(define-presentation-method accept ((type lisp-expression)
				    stream (view textual-view)
				    &key)
  (with-drawing-options (stream :text-size :huge)
...
(define-command (com-eval :menu t :command-table lisp-commands)
  ...
  (let ((r form)
    ...
    (setq time-used (maxima::/ 
		     (float (maxima::- time-after time-before))
		     internal-time-units-per-second)
	  etime-used (maxima::/ 
		      (float (maxima::- etime-after etime-before))
		      internal-time-units-per-second))
    (setq maxima::accumulated-time (maxima::+ maxima::accumulated-time time-used))
...
#+nil
(define-command (com-lisp-eval :menu t :command-table lisp-commands)
    ((form 'clim:form :prompt "form"))
    ;; I don't know why I need to do this, but with my showtime pane
    ;; enabled, *standard-output* is set to the showtime-pane so the
    ;; output here goes to the wrong place.  This sets
    ...
    (let ((*standard-output* (get-frame-pane *application-frame* 'interactor)))
      (with-drawing-options (t :ink +olivedrab+)
	(present (eval form) 'lisp-expression))))
...
(with-drawing-options (stream :text-size :huge)
  (let (object)
    (with-input-editing (stream :input-sensitizer
				(lambda (stream cont)
				  (with-output-as-presentation 
				      (stream object 'lisp-expression)
				    (funcall cont))))
      (setq object (accept 'lisp-expression :stream stream :prompt nil
			   :activation-gestures nil))
      (list 'com-lisp-eval object)))))
