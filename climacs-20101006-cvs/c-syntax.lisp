...
(defclass preprocessor-directive (form c-nonterminal) ())
     ...
     ,@(loop for name in punctuator-names
	     for real-name = 
			   (intern (concatenate 'string 
						 (string name) "-LEXEME")
                                   #.*package*)
	     collecting `(defclass ,real-name (punctuator-lexeme) ((ink) (face)) ))))
...
(defclass left-angle-bracket-lexeme (punctuator-lexeme opening-delimiter-mixin) ())
(defclass right-angle-bracket-lexeme (punctuator-lexeme closing-delimiter-mixin) ())
...
(defmethod lex ((syntax c-syntax) (state lexer-toplevel-state) scan)
  (macrolet ((fo () `(forward-object scan)))
	...
	(#\# (let ((bolp (beginning-of-line-p scan)))
		   ...
		   (or (make-instance 'preprocessor-start-lexeme) (make-instance 'error-lexeme)))))
...
;;; parse trees
(defclass preprocessor-directive-form (preprocessor-directive) ())
...
(define-syntax-highlighting-rules default-c-highlighting
  (form-lexeme (:face :ink +yellow4+))
  (word-lexeme (:face :ink +green3+))
  (equal-lexeme (:face :ink +khaki+))
  (eq-lexeme (:face :ink +burlywood3+))
  (exclamation-lexeme (:face :ink +tan3+))
  (neq-lexeme (:face :ink +sienna3+))
  (increment-lexeme (:face :ink +cyan+))
  (decrement-lexeme (:face :ink +magenta+))
  (error-symbol (*error-drawing-options*))
  (string-form (*string-drawing-options*))
  (operator (*special-operator-drawing-options*))
  (type-specifier (:face :ink +forest-green+))
  (type-qualifier (:face :ink +light-green+))
  (preprocessor-directive (:face :ink +steel-blue+))
  (character-start-lexeme (:face :ink +red3+))
  (character-end-lexeme (:face :ink +red3+))
  (left-angle-bracket-lexeme (:face :ink +green2+))
  (right-angle-bracket-lexeme (:face :ink +green2+))
  (left-bracket-lexeme (:face :ink +red4+))
  (right-bracket-lexeme (:face :ink +red4+))
  (left-parenthesis-lexeme (:face :ink +slate-blue+))
  (right-parenthesis-lexeme (:face :ink +slate-blue+))
  (left-brace-lexeme (:face :ink +red2+))
  (right-brace-lexeme (:face :ink +red2+))
  (storage-class-specifier (:face :ink +royal-blue+))
  (function-specifier (:face :ink +slate-gray+))
  (line-comment-form (*comment-drawing-options*))
  (long-comment-form (*comment-drawing-options*))
  (comment (*comment-drawing-options*))
  (integer-constant-lexeme (:face :ink +gray60+))
  (floating-constant-lexeme (:face :ink +gray60+)))
