...
(define-command (com-show-directory :name "Show Directory"
  ...
  (multiple-value-bind (query-pathname base-pathname)
    ...
    (with-text-family (t :sans-serif)
      (with-drawing-options (t :text-size 16)
      (invoke-as-heading
       (lambda ()
         (cond
...
(defun automagic-translator (pathname)
  ...
  (cond ((wild-pathname-p pathname)
        ...
        ((directoryp pathname)
         (values `(com-show-directory ,pathname)
                 "Show Directory"
                 (format nil "Show Directory #p~A" pathname)))
        (t
      ...
      (cond ((null values) #+NIL (format t "No values.~%"))
            ...
            (t (do* ((i 0 (1+ i))
               ...
               (with-drawing-options (t :ink +limegreen+)
                 (with-text-style (t (make-text-style nil :italic :large))
                   (format t "~A  " i)))
...
(define-command (com-greet :name "greet"
		  :command-table lisp-commands
		  :menu nil
		  :provide-output-destination-keyword t)
  ((name 'string :prompt "name"))
  (format *standard-output* "hello ~a" (string-downcase name)))
...
(define-command (com-ucs :name "ucs"
		   :command-table lisp-commands
		   :menu nil
		   :provide-output-destination-keyword t)
  ((char 'char))
  (format *standard-input* "~a" (insert (or (string (code-char char)) (code-char char)))))
