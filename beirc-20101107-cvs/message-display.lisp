...
(declaim  (optimize (safety 2) (debug 2) (speed 2)
         (space 2)))

(defvar *max-preamble-length* 1)
     ...
     (multiple-value-bind (message foreground background)
       ...
       (cond (*filter-colors* nil)
             ((equal (car foreground)
                     'normal)
              (setf foreground-color +gray10+
                    background-color (climi::xpm-find-named-color "SlateGray")))
             ((equal (car foreground)
                     :ink)
       ...
       (with-drawing-options (*standard-output* :text-size 17) ;;this is the place to change message-body text-sizes
    ...
    (let ((record (with-new-output-record (t)
      ...
      (with-bounding-rectangle* (left top right bottom)
        ...
        (unless (equal left right)
          (unless (equal ,background +gray10+)
  ...
  (with-drawing-options
      (*standard-output*
       :ink (if (string-equal "localhost" (irc:host message))
		(climi::xpm-find-named-color "SlateGray")
		(climi::xpm-find-named-color "wheat4"))
       :text-size :normal)
    (unless (message-from-ignored-nick-p message receiver)
      (with-text-size (*standard-output* :large)
      (with-text-face
	  (*standard-output*
	   (if (message-from-focused-nick-p message receiver) :roman :roman))
...
(defmethod print-message ((message irc:ctcp-version-message) receiver)
  (let ((source (cl-irc:source message)))
    (formatting-message (t message receiver)
      ((format t "    "))
      ((with-drawing-options (*standard-output* :ink (climi::xpm-find-named-color "wheat3") :text-size :small)
         (present source 'unhighlighted-nickname)
         (format t " ")
		...
		,@(loop for (message-type . message-name) in message-specs
		    ...
		    `(defmethod print-message ((message ,message-type) receiver)
			 ...
			 (formatting-message (t message receiver)
			   ...
			   ((with-drawing-options (*standard-output* :ink (climi::xpm-find-named-color "wheat4") :text-size :large)
			      (format-message*
...
(defmethod print-message (message receiver)
  ...
  (irc:destructuring-arguments (&whole args &rest :ignored &req body) message
   (formatting-message (t message receiver)
     ((format t "!!! ~A" (irc:source message)))
     ((with-drawing-options (*standard-output* :ink +red+ :text-size :large)
...
(defmethod print-message ((message irc:irc-quit-message) receiver)
  (irc:destructuring-arguments (&optional body) message
    (formatting-message (t message receiver)
      ((format t "   "))
      ((with-drawing-options (*standard-output* :ink (climi::xpm-find-named-color "SlateGray") :text-size :normal)
         (format t "Quit: ")
...
(defmethod print-message ((message irc:irc-nick-message) receiver)
  (irc:destructuring-arguments (&rest :ignored &req body) message
   (formatting-message (t message receiver)
     ((format t "   "))
     ((with-drawing-options (*standard-output* :ink (climi::xpm-find-named-color "burlywood2") :text-size :small)
...
(defmethod print-message ((message irc:irc-rpl_whoisuser-message) receiver)
  (formatting-message (t message receiver)
    ((format t "   "))
    ((with-drawing-options (*standard-output* :ink (climi::xpm-find-named-color "wheat4") :text-size :small)
...
(defun print-topic (receiver message sender channel topic)
  (formatting-message (t message receiver)
    ((format t "    "))
    ((with-drawing-options (*standard-output* :ink (climi::xpm-find-named-color "SlateBlue") :text-size :normal)
       (cond
...
(defmethod print-message ((message irc:irc-rpl_topicwhotime-message) receiver)
  (formatting-message (t message receiver)
    ((format t "   "))
    ((with-drawing-options (*standard-output* :ink (climi::xpm-find-named-color "SlateGray") :text-size :small)
...
(defmethod print-message ((message irc:irc-join-message) receiver)
  (formatting-message (t message receiver)
    ((format t "   "))
    ((with-drawing-options (*standard-output* :ink (climi::xpm-find-named-color "DarkSlateGray") :text-size :normal)
       (format t "Join: ")
       (present (irc:source message) 'nickname)
       (write-char #\Space)
       (present-as-hostmask (irc:user message) (irc:host message))))))
    ...
    (t
     (irc:destructuring-arguments (target &rest args) message
       (let* ((connection (irc:connection message))
         ...
         (formatting-message (t message receiver)
           ((format t "   "))
           ((with-drawing-options (*standard-output* :ink (climi::xpm-find-named-color "wheat2") :text-size :small)
              (present (irc:source message) 'nickname)
